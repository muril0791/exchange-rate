<template>
  <div style="border-radius: 20px; border: 1px solid whitesmoke">
    <v-card-title class="headline text-center">Shopping car</v-card-title>
    <v-container>
      <v-row>
        <v-col cols="12" md="6">
      ...
      <v-list class="mt-2 pa-2" style="background: none; border: 1px solid grey; border-radius: 20px;"
        ...
        <v-list-item-group style="background: none;">
          <v-list-item v-for="(item, index) in cart" :key="index">
            <v-list-item-title>{{ item.name }}: {{ item.priceY }} {{ currencyY }} ({{
              item.priceX
              }}
              {{ currencyX }})</v-list-item-title>
          </v-list-item>
        </v-list-item-group>
      </v-list>
...
export default {
  ...
  computed: {
    totalY() {
      return this.cart
        .reduce((acc, item) => acc + parseFloat(item.priceY), 0)
        .toFixed(2);
    },
    totalX() {
      return this.cart
        .reduce(
          (acc, item) => acc + parseFloat(item.priceY) / this.exchangeRateXtoY,
          0
        )
        .toFixed(2);
    },
